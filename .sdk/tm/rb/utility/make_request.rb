...
module InternetdbUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = InternetdbResponse.new({})
    ...
    if fetch_err
      ...
      response = InternetdbResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = InternetdbResponse.new(fetched)
    else
