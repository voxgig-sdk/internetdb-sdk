...
require_relative 'Internetdb_types'
...
class InternetdbSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = InternetdbHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = InternetdbHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, InternetdbFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    InternetdbUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = InternetdbHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = InternetdbHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = InternetdbHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = InternetdbHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = InternetdbError.new(
        "graphql_error", "InternetdbSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
