...
class InfoIpGetDirectTest < Minitest::Test
  def test_direct_list_info_ip_get
    setup = info_ip_get_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-info_ip_get", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["info_ip_get01"].each do |_live_key|
...
def info_ip_get_direct_setup(mockres)
  ...
  if live
    ...
    client = InternetdbSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
