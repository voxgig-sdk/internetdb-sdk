...
class InfoIpGetEntity
  ...
  def make
    opts = @_entopts.dup
    InfoIpGetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = InternetdbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [InfoIpGet, Hash] the current InfoIpGet data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = InternetdbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of InfoIpGet fields)
  def match_get
  ...
  #
  # @param reqmatch [InfoIpGetListMatch, Hash, nil] match filter (any subset of
  #   InfoIpGet fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<InfoIpGet>, Array] the matching InfoIpGet items; raises InternetdbError on failure
