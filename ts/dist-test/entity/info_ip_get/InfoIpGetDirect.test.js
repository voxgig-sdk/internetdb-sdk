...
(0, node_test_1.describe)('InfoIpGetDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.InternetdbSDK({
    ...
    (0, node_test_1.test)('direct-list-info_ip_get', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-info_ip_get', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["info_ip_get01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['info_ip_get01'];
        }
        else {
            params.id = 'direct01';
        }
...
function liveScenariosActive() { return false && process.env.INTERNETDB_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.InternetdbSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['INTERNETDB_TEST_INFO_IP_GET_ENTID'];
    ...
    const client = new __1.InternetdbSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=InfoIpGetDirect.test.js.map