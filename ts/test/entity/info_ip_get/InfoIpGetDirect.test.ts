...
describe('InfoIpGetDirect', async () => {
  ...
  test('direct-list-info_ip_get', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-info_ip_get', setup.live)) return
    if (skipIfMissingIds(t, setup, ["info_ip_get01"])) return
