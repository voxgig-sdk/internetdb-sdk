...
//     (\`new InternetdbSDK(...)\` / \`InternetdbSDK.test(...)\` become
//     \`InternetdbSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"info_ip_get":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new InternetdbSDK(...)\` and any
// \`InternetdbSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+InternetdbSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/InternetdbSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
