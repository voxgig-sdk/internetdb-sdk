...
from internetdb_sdk.internetdb_types import (
    InfoIpGet,
    InfoIpGetListMatch,
)
...
class InfoIpGetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return InfoIpGetEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[InfoIpGet]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.InfoIpGet().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
