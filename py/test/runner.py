...
class InternetdbTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            InternetdbTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = InternetdbTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = InternetdbTestRunner.getenv("INTERNETDB_TEST_LIVE")
        override = InternetdbTestRunner.getenv("INTERNETDB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = InternetdbTestRunner.getenv(key)
        ...
        explain = InternetdbTestRunner.getenv("INTERNETDB_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["INTERNETDB_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if InternetdbTestRunner._test_control is not None:
            return InternetdbTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                InternetdbTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            InternetdbTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = InternetdbTestRunner.load_test_control()
...
def load_env_local():
    InternetdbTestRunner.load_env_local()


def env_override(m):
    return InternetdbTestRunner.env_override(m)


def entity_data(v):
    return InternetdbTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return InternetdbTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return InternetdbTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return InternetdbTestRunner.load_test_control()


def live_client_options():
    return InternetdbTestRunner.live_client_options()


def live_delay_ms():
    return InternetdbTestRunner.live_delay_ms()
