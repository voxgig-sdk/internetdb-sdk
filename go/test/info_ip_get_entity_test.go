...
func TestInfoIpGetEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.InfoIpGet(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.InfoIpGet(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		infoIpGetRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.info_ip_get")))
