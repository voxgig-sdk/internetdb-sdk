...
import (
	...
	omni "github.com/voxgig-sdk/internetdb-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.InternetdbError); is {
				out := map[string]any{
					"name":    "InternetdbError",
...
func omniCtx(arg any, client *sdk.InternetdbSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.InternetdbSDK); is {
			client = live
