...
boru-driven command-line client **and** interactive REPL for the Internetdb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/internetdb-cli)
make build
...
./internetdb-cli list info_ip_get

# 5. Override the API base URL for a single call
INTERNETDB_BASE=https://api.example.com ./internetdb-cli list info_ip_get

# 6. No arguments -> interactive REPL
./internetdb-cli
internetdb> list info_ip_get
internetdb> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/internetdb-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/internetdb-cli list info_ip_get
   ```
...
Run with no arguments to open a REPL (prompt `internetdb>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `info_ip_get`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/internetdb-cli`. |
...
(`./internetdb-cli <expr>`) and interactively (the REPL), and expressions compose the
