// Command Internetdb-cli is a CLI + REPL for the Internetdb SDK,
...
const prompt = "internetdb"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "info_ip_get"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      INTERNETDB_APIKEY (api key), INTERNETDB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: INTERNETDB_APIKEY carries the API key and
	// INTERNETDB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("INTERNETDB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("INTERNETDB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewInternetdbSDK(opts)
