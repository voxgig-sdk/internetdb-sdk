-- Internetdb SDK InfoIpGet entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InfoIpGetEntity = {}
InfoIpGetEntity.__index = InfoIpGetEntity


function InfoIpGetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InfoIpGetEntity)
  self._name = "info_ip_get"
  self._client = client
...
function InfoIpGetEntity:get_name()
  return self._name
end


function InfoIpGetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InfoIpGetEntity.new(self._client, opts)
end
...
function InfoIpGetEntity:mark_deleted()
  self._deleted = true
end


function InfoIpGetEntity:deleted()
  return true == self._deleted
end


function InfoIpGetEntity:data_set(args)
  if args ~= nil then
...
function InfoIpGetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InfoIpGetEntity:match_set(args)
  if args ~= nil then
...
function InfoIpGetEntity:match_get()
...
function InfoIpGetEntity:stream(action, args, callopts)
...
---@param reqmatch InfoIpGetListMatch
---@param ctrl? table
---@return InfoIpGet[]
---@return string? err
function InfoIpGetEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function InfoIpGetEntity:_run_op(ctx, post_done)
  local utility = self._utility
