...
describe("InfoIpGetDirect", function()
  it("should direct-list-info_ip_get", function()
    local setup = info_ip_get_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-info_ip_get", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"info_ip_get01"}) do
