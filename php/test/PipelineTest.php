...
require_once __DIR__ . '/../internetdb_sdk.php';
...
{
    private static function utility(): InternetdbUtility
    {
        return new InternetdbUtility();
    }
    ...
    private static function ctx(array $over = []): InternetdbContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new InternetdbContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new InternetdbOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof InternetdbError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new InternetdbOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = InternetdbMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new InternetdbOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = InternetdbMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new InternetdbOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = InternetdbMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new InternetdbSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = InternetdbMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new InternetdbResponse([]),
            'result' => new InternetdbResult([]),
        ]);
        [, $err] = InternetdbMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new InternetdbSpec([]),
            'response' => null,
            'result' => new InternetdbResult([]),
        ]);
        [, $err] = InternetdbMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new InternetdbSpec(['step' => 's']),
            'response' => new InternetdbResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new InternetdbResult([]),
        ]);
        [, $err] = InternetdbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new InternetdbSpec(['step' => 's']),
            'response' => new InternetdbResponse(self::resp(200, ['v' => 1])),
            'result' => new InternetdbResult([]),
        ]);
        [, $err] = InternetdbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new InternetdbSpec(['step' => 's']),
            'response' => new InternetdbResponse(self::resp(200, ['v' => 2])),
            'result' => new InternetdbResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        InternetdbMakeResponse::call($ctx);
    ...
    {
        $preset = new InternetdbResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new InternetdbSpec([]),
            'response' => new InternetdbResponse([]),
            'result' => new InternetdbResult([]),
        ...
        [$got, $err] = InternetdbMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new InternetdbResult([])]);
        [, $err] = InternetdbMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new InternetdbSpec([]), 'result' => null]);
        [, $err] = InternetdbMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new InternetdbSpec(['step' => 's']),
            'result' => new InternetdbResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new InternetdbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = InternetdbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new InternetdbSpec(['step' => 's']),
            'result' => new InternetdbResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new InternetdbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = InternetdbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new InternetdbResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new InternetdbSpec([]),
            'result' => new InternetdbResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = InternetdbMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = InternetdbMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new InternetdbError('boom', 'boom');
        $utility->fetcher = function (InternetdbContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new InternetdbSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = InternetdbMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (InternetdbContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = InternetdbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(InternetdbResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        InternetdbMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (InternetdbContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new InternetdbSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = InternetdbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new InternetdbResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new InternetdbSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = InternetdbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = InternetdbMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = InternetdbMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new InternetdbResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, InternetdbDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new InternetdbResult(['ok' => false])]);
        $this->expectException(InternetdbError::class);
        InternetdbDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new InternetdbResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', InternetdbMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        InternetdbMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new InternetdbBaseFeature();
        $b = new InternetdbBaseFeature();
        InternetdbFeatureAdd::call($ctx, $a);
        InternetdbFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): InternetdbBaseFeature
    {
        $f = new InternetdbBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        InternetdbFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): InternetdbContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new InternetdbSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = InternetdbPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        InternetdbPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new InternetdbResponse(['headers' => null]),
            'result' => new InternetdbResult([]),
        ]);
        InternetdbResultHeaders::call($ctx);
