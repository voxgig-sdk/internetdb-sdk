...
class FtCtrl extends InternetdbControl
{
...
class FtClient
{
    ...
    private InternetdbUtility $utility;

    public function __construct(InternetdbUtility $utility, array $options)
    {
    ...
    public function get_utility(): InternetdbUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (InternetdbContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public InternetdbContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (InternetdbContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = InternetdbConfig::make_feature($name);
    ...
    private static function buildUrl(InternetdbSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof InternetdbError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new InternetdbResponse($fetched);
        }
    ...
    private function fail(InternetdbContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (InternetdbContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
