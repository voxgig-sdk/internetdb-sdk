...
class InternetdbSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = InternetdbHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = InternetdbHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!InternetdbFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, InternetdbFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return InternetdbUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = InternetdbHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = InternetdbHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = InternetdbHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new InternetdbError($op . "_allow",
                "InternetdbSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = InternetdbHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new InternetdbError("graphql_error",
                "InternetdbSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function InfoIpGet($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_info_ip_get === null) {
                $this->_info_ip_get = new InfoIpGetEntity($this, null);
            }
            return $this->_info_ip_get;
        }
        return new InfoIpGetEntity($this, $data);
    }
