...
[MCP](https://modelcontextprotocol.io) server exposing the Internetdb SDK as
two agent tools — `internetdb_list` and `internetdb_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/internetdb-mcp)
make build

# 2. Provide credentials via the environment
export INTERNETDB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user internetdb \
  -- /absolute/path/to/internetdb-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./internetdb-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// internetdb_list: first page of records
{ "entity": "info_ip_get" }
{ "entity": "info_ip_get", "query": { } }

// internetdb_load: one record by id
{ "entity": "info_ip_get", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/internetdb-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user internetdb \
     -- "$PWD"/dist/*/internetdb-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `internetdb_list` and `internetdb_load` tools now appear
   in new sessions. Ask the agent to *"list info_ip_get using internetdb"*
   and it calls `internetdb_list` with `{"entity":"info_ip_get"}`.
...
```sh
export INTERNETDB_APIKEY=sk_live_xxx            # API key
export INTERNETDB_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `internetdb_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `internetdb_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `INTERNETDB_APIKEY` | API key sent with every request. |
| `INTERNETDB_BASE` | Optional override of the API base URL. |
...
```sh
./internetdb-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"internetdb_load","arguments":{"entity":"info_ip_get","query":{"id":1}}}}'
```
